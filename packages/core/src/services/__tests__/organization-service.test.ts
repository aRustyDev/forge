...
describe('OrganizationService', () => {
  ...
  test('create with whitespace-only name fails validation', () => {
    const result = service.create({ name: '   ' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.error.code).toBe('VALIDATION_ERROR')
  ...
  test('create with valid org_type succeeds', () => {
    const result = service.create({ name: 'DoD', org_type: 'government' })
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.data.org_type).toBe('government')
  })

  test('create defaults org_type to company', () => {
    const result = service.create({ name: 'Startup Inc' })
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.data.org_type).toBe('company')
  })
  ...
  test('get returns existing org', () => {
    const created = service.create({ name: 'Test Corp' })
  ...
  test('list returns all orgs with pagination', () => {
  ...
  test('list filters by org_type', () => {
    service.create({ name: 'Company A', org_type: 'company' })
    service.create({ name: 'School B', org_type: 'education' })
    service.create({ name: 'Company C', org_type: 'company' })

    const result = service.list({ org_type: 'education' })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test('list filters by worked', () => {
    service.create({ name: 'Worked Here', worked: 1 })
    service.create({ name: 'Never Worked' })

    const result = service.list({ worked: 1 })
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.data).toHaveLength(1)
  ...
  test('update with invalid org_type fails validation', () => {
    const created = service.create({ name: 'Test' })
    if (!created.ok) return
    const result = service.update(created.data.id, { org_type: 'nope' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.error.code).toBe('VALIDATION_ERROR')
  })

  test('update returns NOT_FOUND for missing org', () => {
    const result = service.update('nonexistent-id', { name: 'Whatever' })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.error.code).toBe('NOT_FOUND')
  })
  ...
  test('delete returns NOT_FOUND for missing org', () => {
